module GooglePlaces
  class Spot
    ...
    def self.list(lat, lng, api_key, options = {})
      ...
      radius = options.delete(:radius) || (rankby ? nil : 200)
      ...
      # Use the rankby in the case that is present
      options[:rankby] = rankby if(rankby)
      # ignore the radius in the case that we have a valid rankby
      options[:radius] = radius if(rankby.nil?)
